...
def get_sliced_prediction_batch(
    ...
    Args:
        ...
        verbose: int
            0: no print
            1: print number of slices and batch info (default)
            2: print number of slices, batch info, and slice/prediction durations
        batch_size: Optional[int]
            Number of slices to process in each batch. If None, process all slices at once.
            Default: None.

    Returns:
        PredictionResult containing object_prediction_list and durations_in_seconds
    """
    # For profiling
    durations_in_seconds = dict()
    ...
    # Set batch_size: if None, process all slices at once
    ...
    if verbose == 1 or verbose == 2:
        print(f"Performing prediction on {num_slices} slices with batch size {batch_size}.")

    # Process each batch
    for i in range(num_batches):
        start_idx = i * batch_size
        end_idx = min((i + 1) * batch_size, num_slices)
        batch_images = slice_image_result.images[start_idx:end_idx]
        batch_shift_amounts = slice_image_result.starting_pixels[start_idx:end_idx]

        # Perform prediction on the batch
        prediction_result = get_prediction_batched(
            image=batch_images,
            detection_model=detection_model,
            shift_amount_list=batch_shift_amounts,
            full_shape=[
        ...
        total_prediction_time += prediction_result.durations_in_seconds["prediction"]

    # Merge matching predictions
    if len(final_object_prediction_list) > 1:
        final_object_prediction_list = postprocess(final_object_prediction_list)

    # Update total prediction time
    durations_in_seconds["prediction"] = total_prediction_time

    # Verbose output for durations
    if verbose == 2:
        print(f"Slicing performed in {durations_in_seconds['slice']} seconds.")
        print(f"Prediction performed in {durations_in_seconds['prediction']} seconds.")

    return PredictionResult(
        object_prediction_list=final_object_prediction_list,
